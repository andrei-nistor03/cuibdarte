    ...
    >
      <div ref={parallaxRef} style={{ willChange: "transform" }}>
        <button
          ...
          className={`group relative block w-full cursor-pointer rounded-xs bg-[#fdf8ee] p-3 pb-9 transition-[transform,box-shadow,opacity,filter] duration-300 ease-out active:scale-[0.98] focus-visible:outline focus-visible:outline-offset-4 focus-visible:outline-(--red) motion-reduce:transition-none sm:p-3.5 sm:pb-11 ${
            active
...
function ArchiveLabel({
  ...
  return (
    <div
      ref={labelRef}
      className="relative mt-10 px-6 text-center md:mt-12 lg:mt-8"
    >
      <p className="section-hand -rotate-1 text-2xl leading-snug text-(--ink)/90 sm:text-3xl lg:text-4xl">
  ...
  useEffect(() => {
    ...
    if (!hasMorphed.current && !reduced) {
      ...
      const scale = Math.min(
        1,
        Math.max(0.18, originRect.width / finalRect.width),
      );
      gsap.fromTo(
        frameRef.current,
        {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <p className="section-hand absolute bottom-4 left-0 right-0 -rotate-1 px-4 text-center text-xl text-(--ink)/85 sm:bottom-5 sm:text-2xl">
          {photo.caption}
        </p>
      </div>
    </div>
...
export default function Memories() {
  ...
  const handleNavigate = useCallback((dir: 1 | -1) => {
    setLightbox((cur) =>
      cur === null
        ? cur
        : {
            index: (cur.index + dir + photoMeta.length) % photoMeta.length,
            rect: cur.rect,
          },
    );
  }, []);
        ...
        photoMeta.forEach((p, i) => {
          ...
          gsap.fromTo(
            el,
            {
              y: 26,
              opacity: 0,
              rotation: p.rotate * 0.6 + (i % 2 === 0 ? 4 : -4),
            },
            {
              y: 0,
              opacity: 1,
  ...
  return (
    <section
      id="memories"
      ref={sectionRef}
      className="relative scroll-mt-20 overflow-hidden bg-(--paper) pt-14 pb-28 md:pt-16 md:pb-40 lg:pt-10 lg:pb-56"
    >
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(to bottom, transparent, rgba(36,24,17,0.35))",
        }}
      />
